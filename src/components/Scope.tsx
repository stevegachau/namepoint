...
export function Scope() {
	return (
		...
		>
			<div className="container mx-auto py-20 md:py-28">
				<div className="grid items-center gap-x-16 gap-y-12 lg:grid-cols-2">
					{/* text half */}
					<div className="lg:pr-8">
						<div>
							<h2 className="text-[2.1rem] font-medium leading-[1.05] tracking-tighter md:text-[3.2rem]">
  ...
  `}
							...
							<p className="mt-8 max-w-md text-sm leading-relaxed text-muted-foreground">
								...
								here or in any ENS manager. If this site went offline your name
					...
					<div className="shadow-soft overflow-hidden rounded-md border border-border bg-card">
						{GROUPS.map((g, gi) => (
							<div key={g.label}>
								<div className={gi > 0 ? "border-t border-border" : ""}>
									<div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-border bg-secondary/30 px-5 py-3">
									...
									<dl className="px-5">
										{g.rows.map((r) => (
											<div
												key={r.key}
												className="flex items-baseline justify-between gap-6 border-b border-border/50 py-3 last:border-0"
											>
												<dt
